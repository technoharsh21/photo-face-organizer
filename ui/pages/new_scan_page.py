...
class ProfileSelectionItemWidget(QWidget):
    """Custom rich item card for selecting target profiles in Step 2 with clear typography."""
    ...
    def __init__(self, profile_id: str, name: str, ref_count: int, avatar_path: str | None, is_group: bool, is_checked: bool, on_toggled: Callable[[str, bool], None], bg_color: str = "#2563eb"):
        ...
        # Text Info with clear, prominent styling
        ...
        sub_tag = "👥 Group Profile" if is_group else f"📷 {ref_count} reference photo{'s' if ref_count != 1 else ''}"
...
class NewScanPage(QWidget):
    """Modern, responsive 5-step wizard for configuring and launching photo organization scans."""
    ...
    def __init__(
        ...
        self.default_output_path: str = str(Path.home() / "Pictures" / "Organized_Photos")
    ...
    def _setup_ui(self):
        ...
        page_title = QLabel("🚀 New Scan Wizard")
        page_title.setStyleSheet("font-size: 18px; font-weight: 800; color: #ffffff;")
        page_sub = QLabel("Organize & sort photos by facial identity with InsightFace AI (512-d embeddings).")
        page_sub.setStyleSheet("color: #94a3b8; font-size: 12px;")
        ...
        self.btn_start = QPushButton("🚀 Start AI Scan")
        self.btn_start.setProperty("class", "PrimaryButton")
        self.btn_start.setCursor(Qt.PointingHandCursor)
        self.btn_start.setFixedHeight(40)
        self.btn_start.setStyleSheet(
            "QPushButton { background-color: #10b981; color: #ffffff; font-weight: 800; border-radius: 8px; padding: 0 24px; font-size: 13px; border: 1px solid #059669; min-width: 140px; }"
    ...
    def _create_step1_sources(self) -> QWidget:
        ...
        lbl_h = QLabel("<b>Step 1: Choose Photo Sources to Scan</b>")
        lbl_h.setStyleSheet("font-size: 15px; color: #ffffff;")
        lbl_sub = QLabel("Select folders (e.g. Pictures, Camera Roll, Hard Drives) or individual image files you wish to sort.")
    ...
    def _create_step2_profiles(self) -> QWidget:
        ...
        hdr_box = QVBoxLayout()
        hdr_box.setSpacing(4)
        lbl_h = QLabel("<b>Step 2: Select People Profiles to Match Against</b>")
        lbl_h.setStyleSheet("font-size: 15px; color: #ffffff;")
        lbl_sub = QLabel("Photos containing matched faces will be sorted into respective person folders.")
        lbl_sub.setStyleSheet("font-size: 12px; color: #94a3b8;")
        hdr_box.addWidget(lbl_h)
        hdr_box.addWidget(lbl_sub)
        l.addLayout(hdr_box)
    ...
    def _create_step3_output(self) -> QWidget:
        ...
        hdr_box = QVBoxLayout()
        hdr_box.setSpacing(4)
        lbl_h = QLabel("<b>Step 3: Destination Folder & File Handling Mode</b>")
        lbl_h.setStyleSheet("font-size: 15px; color: #ffffff;")
        lbl_sub = QLabel("Specify where organized photos will be saved and whether originals should be preserved.")
        lbl_sub.setStyleSheet("font-size: 12px; color: #94a3b8;")
        hdr_box.addWidget(lbl_h)
        hdr_box.addWidget(lbl_sub)
        l.addLayout(hdr_box)
        ...
        oc_layout.addWidget(QLabel("<b>Output Directory:</b>"))
        ...
        copy_lbl = QLabel(
            ...
            "Safely copies matched photos into organized person folders. Original source photos remain 100% untouched."
            "</span>"
        )
        ...
        # Explicit QButtonGroup to enforce mutual exclusivity across different parent containers
    ...
    def _create_step4_performance(self) -> QWidget:
        ...
        hdr_box = QVBoxLayout()
        hdr_box.setSpacing(4)
        lbl_h = QLabel("<b>Step 4: AI Matching Sensitivity & Performance</b>")
        lbl_h.setStyleSheet("font-size: 15px; color: #ffffff;")
        lbl_sub = QLabel("Tune matching threshold precision and select hardware processing preferences.")
        lbl_sub.setStyleSheet("font-size: 12px; color: #94a3b8;")
        hdr_box.addWidget(lbl_h)
        hdr_box.addWidget(lbl_sub)
        l.addLayout(hdr_box)
        ...
        # 2. Matching Threshold Precision Card
        thresh_card = QFrame()
        ...
        self.spin_threshold.setValue(55)
        ...
        self._update_threshold_guidance(55)
    ...
    def _update_threshold_guidance(self, value: int):
        if value >= 85:
            ...
            text = (
                f"🎯 <b>{value}% (High Precision):</b> Highly accurate matching. "
            ...
            text = (
                f"⚖️ <b>{value}% (Balanced Mode — Recommended):</b> Standard matching for solo & group photos. "
                f"Captures your face across varied smiles, hairstyles, and lighting."
            )
    ...
    def _create_step5_review(self) -> QWidget:
        ...
        hdr_box = QVBoxLayout()
        hdr_box.setSpacing(4)
        lbl_h = QLabel("<b>Step 5: Review Configuration & Launch Scan</b>")
        lbl_h.setStyleSheet("font-size: 15px; color: #ffffff;")
        lbl_sub = QLabel("Verify your scan parameters before starting the InsightFace neural network.")
        lbl_sub.setStyleSheet("font-size: 12px; color: #94a3b8;")
        hdr_box.addWidget(lbl_h)
        hdr_box.addWidget(lbl_sub)
        l.addLayout(hdr_box)
        ...
        p_title = QLabel("👥 <b>Target People</b>")
        ...
        e_title = QLabel("🧠 <b>AI Neural Engine</b>")
        e_title.setStyleSheet("color: #fbbf24; font-size: 13px;")
        self.lbl_rev_thresh_val = QLabel("Threshold: 55%")
        self.lbl_rev_thresh_val.setStyleSheet("color: #ffffff; font-size: 14px; font-weight: bold;")
        self.lbl_rev_engine_sub = QLabel("InsightFace SCRFD + ArcFace (512-d)")
        ...
        lbl_ready = QLabel("✅ <b>Configuration Ready:</b> Click '🚀 Start AI Scan' below to start organizing your photos.")
    ...
    def reset_wizard(self):
        """Cleanly reset all 5 wizard steps and clear cached values from previous scans."""
        # 1. Reset Sources
        self.sources.clear()
    ...
    def _populate_profiles_list(self):
        ...
        for idx, p in enumerate(profiles):
            ...
            widget = ProfileSelectionItemWidget(
                profile_id=p_id,
    ...
    def _filter_profiles_list(self, query: str):
        ...
        for i in range(self.profiles_list_widget.count()):
            ...
            if isinstance(w, ProfileSelectionItemWidget):
    ...
    def _select_output_dir(self):
        folder = QFileDialog.getExistingDirectory(self, "Select Destination Folder for Organized Photos")
        if folder:
    ...
    def _populate_review(self):
        ...
        self.lbl_rev_people_val.setText(f"{len(sel_names)} Profile{'s' if len(sel_names) != 1 else ''} Selected")
        ...
        self.lbl_rev_engine_sub.setText("InsightFace SCRFD + ArcFace (512-d)")
